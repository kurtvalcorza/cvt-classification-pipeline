"""Static release-asset validation for the CvT-13 image-classification DIMER pipeline.
...
EXPECTED_MODEL_ID = "microsoft/cvt-13"
WEIGHTS_KEY = "cvt-13"
...
EXPECTED_OUTPUTS = (
    "cvt_classification_input_manifest.json",
    "cvt_classification_evaluation_report.json",
    "cvt_classification_result.json",
    "cvt_classification_predictions.csv",
    "cvt_adapter.safetensors",
)
...
CODE_MARKERS = (
    ...
    "('blank', blank_image()), ('noise', noise_image(0))",
...
MARKDOWN_MARKERS = (
    ...
    "**Convolutions inside the transformer.**",
)
...
def validate_pin_state() -> None:
    ...
    _check(
        manifest.get("totalBytes") == sum(entry["bytes"] for entry in manifest["files"]),
        "manifest totalBytes != sum of file bytes",
    )
    ...
    if revision == UNPINNED:
        _check(
            all(d is None for d in digests),
            "an unpinned manifest must not carry digests; run tools/pin_snapshot.py to pin every file at one commit",
        )
        for name in docs:
            _check(
        ...
        _check(
            all(isinstance(d, str) and re.fullmatch(r"[0-9a-f]{64}", d) for d in digests),
            "a pinned manifest must record a SHA-256 for every file",
        )
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    986707: "sample dataset archive CIFAR-10-subset.zip (Cleanlab/cifar-10-subset at the pinned commit)",
}
EXTERNAL_WEIGHT_DIGESTS: dict[str, str] = {
    "66f90a4f87d865e8eb653b62f10e754684075a32314177de76832349d4b1fb19": "sample dataset archive CIFAR-10-subset.zip SHA-256",
}
