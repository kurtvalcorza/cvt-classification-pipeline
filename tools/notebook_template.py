...
TEMPLATE = {
    "package": "cvt_classification_pipeline",
    "repo_name": "cvt-classification-pipeline",
    "stem": "cvt_classification",
    "notebook_name": "cvt_classification_colab.ipynb",
    "profile": "E2E",
    "mode": "GUIDED",
    "pipeline_class": "CvtPipeline",
    "weights_key": "cvt-13",
    "modules": [
        "data.py",
        "pipeline.py",
    ...
    "runtime_imports": ["torch", "torchvision", "transformers", "numpy", "PIL"],
    "title": "CvT-13 (ImageNet-1k, 224 px) — DIMER image classification and bounded fine-tuning (standalone)",
    "badges": [
        (
            "GitHub",
            "https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white",
            "https://github.com/kurtvalcorza/cvt-classification-pipeline",
        ),
        (
            "Open In Colab",
            "https://colab.research.google.com/assets/colab-badge.svg",
            "https://colab.research.google.com/github/kurtvalcorza/cvt-classification-pipeline/blob/main/tutorials/cvt_classification_colab.ipynb",
        ),
        (
            "Hugging Face",
            "https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-microsoft%2Fcvt--13-ffcc4d?style=flat",
            "https://huggingface.co/microsoft/cvt-13",
        ),
        (
            "Upstream",
            "https://img.shields.io/badge/Upstream-microsoft%2FCvT-181717?style=flat&logo=github&logoColor=white",
            "https://github.com/microsoft/CvT",
        ),
        (
            "arXiv",
            "https://img.shields.io/badge/arXiv-2103.15808-b31b1b.svg",
            "https://arxiv.org/abs/2103.15808",
        ),
        (
            "License",
            "https://img.shields.io/badge/License-Apache--2.0-green.svg",
            "https://github.com/kurtvalcorza/cvt-classification-pipeline/blob/main/LICENSE",
        ),
    ],
    "capability": "ImageNet-1k image classification with CvT-13, a Vision Transformer with convolutional token embeddings and projections, and a bounded fine-tune that replaces the 1000-class head with one for your own classes, compares it on a held-out split with the majority-class and zero-shot ImageNet baselines, and exports a reloadable SafeTensors adapter",
    "intro": (
        "CvT (Convolutional vision Transformer) brings two convolutions into a Vision Transformer. Each of its three stages starts with a "
        "**convolutional token embedding**, an overlapping strided convolution that shrinks the token map and widens the channels, and every "
        "attention layer computes its queries, keys and values through a **convolutional projection** (a depthwise convolution followed by "
        "BatchNorm). The convolutions carry spatial position, so CvT uses no position embeddings. CvT-13 has 1, 2 and 10 transformer blocks at "
        "widths 64, 192 and 384, about 20M parameters, and a `[CLS]` token in its last stage whose normalised output feeds a linear head over "
        "the 1000 ImageNet classes. Preprocessing follows the checkpoint's `preprocessor_config.json`: bicubic resize of the shorter side to "
        "256 px, a 224 px centre crop, and the ImageNet mean and standard deviation.\n\n"
        "**Convolutions inside the transformer.** Because the projections contain BatchNorm, CvT keeps running statistics that a frozen "
        "fine-tune must hold fixed; Section 8 shows how.\n\n"
    ...
    "prerequisites": [
        "- **Runtime:** a fresh supported runtime (Google Colab or Jupyter, Python 3.12). A CUDA GPU such as a Colab or Kaggle T4 is the documented runtime for the fine-tuning stages and is used automatically when present; the notebook also runs on CPU, more slowly. Runtimes are not measured in this revision. The pinned `torch==2.14.0` wheel is the largest download; the checkpoint is about 80 MB.",
        "- **Knowledge:** basic Python and PIL; what a softmax over classes is; the idea of an image transformer operating on tokens; accuracy, balanced accuracy and a confusion matrix; why a baseline is needed before a score means anything.",
    ...
    "cells": [
        ...
        # ---------------------------------------------------------------- 7. Re-head & baseline
        {
            "md": (
                ...
                "(`classifier`, 384 features to 1000 classes) with a new 384-to-2 layer initialised under `SEED`. Every other weight keeps its "
                "ImageNet value.\n\n"
            ...
            "code": (
                "adapter = CvtPipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=SAMPLE_CLASSES, seed=SEED)\n"
        ...
        # ---------------------------------------------------------------- 8. Fine-tune
        {
            "md": (
                ...
                "- **What trains:** with `FREEZE_BACKBONE = False` (the default) every weight trains. With `True`, only the new head trains and "
                "the backbone (`cvt.` and the final `layernorm.`) keeps its weights; the BatchNorm layers in the convolutional projections are held in evaluation mode, so their running statistics stay fixed too. The cell "
                "prints the trainable and total parameter counts.\n"
        ...
        # ---------------------------------------------------------------- 11. Export, reload & verify
        {
            "md": (
                ...
                "`save_artifact` writes `outputs/cvt_adapter.safetensors`: every tensor the fine-tune could change, plus a metadata header "
            ...
            "code": (
                "artifact_path = OUTPUTS / 'cvt_adapter.safetensors'\n"
                "descriptor = adapter.save_artifact(artifact_path, notes='CvT-13 CIFAR-10 frog/truck tutorial adapter')\n"
                "print(json.dumps(descriptor, indent=2))\n\n"
                "reloaded = CvtPipeline.load_artifact(artifact_path, weights_dir=WEIGHTS_DIR)\n"
        ...
        # ---------------------------------------------------------------- 12. Outputs & provenance
        {
            "md": (
                ...
                "The cell writes:\n"
                "- `outputs/cvt_classification_input_manifest.json`\n"
                "- `outputs/cvt_classification_evaluation_report.json`\n"
                "- `outputs/cvt_classification_result.json` (identity, runtime versions, device, dataset provenance and manifest, split, "
                "baselines, fine-tuning configuration, held-out and unseen metrics, adapter descriptor and reload check)\n"
                "- `outputs/cvt_classification_predictions.csv`\n"
                "- `outputs/cvt_adapter.safetensors` (written in Section 11)"
            ),
            "code": (
                "import csv\n\n"
        ...
        # ---------------------------------------------------------------- 13. BYOD
        {
            ...
            "code": (
                ...
                "    byod_pipe = CvtPipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=byod_names, seed=SEED)\n"
                ...
                "    byod_descriptor = byod_pipe.save_artifact(OUTPUTS / 'byod_cvt_adapter.safetensors', notes='BYOD adaptation adapter')\n"
                "    CvtPipeline.load_artifact(OUTPUTS / 'byod_cvt_adapter.safetensors', weights_dir=WEIGHTS_DIR)\n"
                "    print('BYOD adapter exported and reloaded:', byod_descriptor['sha256'][:16])\n"
                "else:\n"
                "    print('BYOD dataset branch is off; set USE_BYOD_DATASET = True to adapt CvT-13 on your own class folders.')"
            ),
        },
    ],
    "closing": (
        "## Interpretation and limits\n\n"
        "**What this notebook established, in this runtime.** The pinned `microsoft/cvt-13` snapshot was verified against a committed "
        ...
        "## References\n\n"
        "- Wu, H., Xiao, B., Codella, N., Liu, M., Dai, X., Yuan, L. and Zhang, L. (2021). *CvT: Introducing Convolutions to Vision Transformers.* ICCV 2021. [arXiv:2103.15808](https://arxiv.org/abs/2103.15808).\n"
        "- Hugging Face checkpoint: [microsoft/cvt-13](https://huggingface.co/microsoft/cvt-13) — Apache-2.0; code: [microsoft/CvT](https://github.com/microsoft/CvT) — MIT.\n"
        ...
        "- Repository model card: https://github.com/kurtvalcorza/cvt-classification-pipeline/blob/main/MODEL_CARD.md\n"
        "- [`kurtvalcorza/cvt-classification-pipeline`](https://github.com/kurtvalcorza/cvt-classification-pipeline) — source repository for this pipeline."
    ),
}
