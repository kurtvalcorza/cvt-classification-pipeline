...
- Executed artifact: `model.safetensors` (80,166,694 bytes as the Hub reported for `main` when this repository was built).
...
- Manifest: `weights/cvt-13/dimer-base-manifest.json` (4 staged files: `README.md`, `config.json`, `preprocessor_config.json`, `model.safetensors`; `totalBytes` 80238966; plus the two reference files). Every `sha256` is `null` until the pin tool runs.
- Committed copy: `preprocessor_config.json` (266 bytes) is committed as the Hub served it, so a test can check the pipeline's torchvision transform against the Transformers processor built from it. The pin tool replaces it with the bytes downloaded at the pinned commit before hashing.
