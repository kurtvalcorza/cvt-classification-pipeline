...
def _copy_repo(tmp_path: Path) -> Path:
    ...
    shutil.copytree(
        ROOT / "weights", tmp_path / "weights", ignore=shutil.ignore_patterns("*.safetensors", "*.bin")
    )
    return tmp_path
...
def _fake_hub(files: dict[str, bytes], lfs_override: dict[str, str] | None = None):
    ...
    def model_info(model_id, revision, files_metadata):
        ...
        siblings = []
        for name, data in {**files, **REFERENCE}.items():
            lfs = None
            if name.endswith((".safetensors", ".bin", ".h5")):
...
FILES = {
    "README.md": b"# card\n",
    "config.json": b'{"model_type": "cvt"}',
    "preprocessor_config.json": b"{}",
    "model.safetensors": b"\x00" * 64,
}
REFERENCE = {"pytorch_model.bin": b"\x02" * 72, "tf_model.h5": b"\x01" * 80}
...
def test_pin_writes_commit_digests_and_module_revision(tmp_path):
    ...
    manifest = json.loads((root / "weights/cvt-13/dimer-base-manifest.json").read_text())
    ...
    module = (root / "src/cvt_classification_pipeline/pipeline.py").read_text()
    assert f'MODEL_REVISION = "{COMMIT}"' in module
    recorded = {r["path"]: (r["sha256"], r["bytes"]) for r in manifest["referenceFiles"]}
    assert recorded == {
        name: (hashlib.sha256(data).hexdigest(), len(data)) for name, data in REFERENCE.items()
    }
    assert not set(REFERENCE) & {name for name, _revision in calls}  # recorded, never downloaded


def test_pin_refuses_a_digest_that_disagrees_with_the_hub(tmp_path):
    root = _copy_repo(tmp_path)
    before = (root / "weights/cvt-13/dimer-base-manifest.json").read_text()
    module_before = (root / "src/cvt_classification_pipeline/pipeline.py").read_text()
    ...
    assert (root / "weights/cvt-13/dimer-base-manifest.json").read_text() == before
    assert (root / "src/cvt_classification_pipeline/pipeline.py").read_text() == module_before
