"""End-to-end adaptation on a small random-weight CvT: no checkpoint, no network.

The model is the same ``CvtForImageClassification`` class the pinned checkpoint loads into, shrunk to
one narrow block per stage on 32 px inputs so a CPU step takes well under a second. These tests
exercise the real prediction, zero-shot, fine-tuning, evaluation, adapter export and adapter reload code
paths. They say nothing
...
from cvt_classification_pipeline import (  # noqa: E402
    ARTIFACT_FORMAT,
    IMAGENET_GROUPS,
    MODEL_ID,
    MODEL_KEY,
...
from cvt_classification_pipeline import pipeline as pipeline_module  # noqa: E402

imagenet_labels = pipeline_module.imagenet_labels


# transformers indexes stage i's drop-path schedule at i, so stage i needs depth >= i + 1 (CvT-13: 1, 2, 10)
SMALL = {"embed_dim": [16, 32, 48], "depth": [1, 2, 3], "num_heads": [1, 2, 2]}


def _imagenet_pipeline(seed: int = 0) -> CvtPipeline:
    torch.manual_seed(seed)
    model = build_model(**SMALL)
    return CvtPipeline.from_components(
        model, class_names=imagenet_labels(model), transform=pipeline_module.eval_transform(image_size=32)
    )


def _adapter_pipeline(seed: int = 0, class_names=SAMPLE_CLASSES) -> CvtPipeline:
...
def test_backbone_is_everything_but_the_classifier():
    assert pipeline_module._backbone_prefixes(build_model(**SMALL)) == ("cvt.", "layernorm.")
...
def test_frozen_finetune_keeps_backbone_weights(tmp_path, data):
    train, held = data
    pipe = _adapter_pipeline()
    ...
    assert any("running_mean" in k for k in frozen)  # CvT's convolutional projections carry BatchNorm
...
def test_apply_artifact_refuses_mismatched_adapters(tmp_path, data):
    ...
    tensor = {"classifier.bias": torch.zeros(2)}
